...
export async function GET(request: NextRequest) {
  ...
  const authParams: OAuth21AuthParams = {
    ...
    client_id: searchParams.get("client_id") || process.env.GOOGLE_CLIENT_ID,
    redirect_uri: searchParams.get("redirect_uri") || `${resolveApiDomain()}/api/auth/callback/google`,
  ...
  if (!searchParams.get("client_id") && process.env.GOOGLE_CLIENT_ID) {
    console.log("Authorize endpoint: Using fallback Google Client ID");
  }

  if (!searchParams.get("redirect_uri")) {
    console.log(`Authorize endpoint: Using fallback redirect URI: ${authParams.redirect_uri}`);
  }
  ...
  const validRedirectUris = [
    ...
    `${baseUrl}/api/auth/callback/google`, // Allow our own callback
  ];
