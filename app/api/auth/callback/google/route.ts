  ...
  try {
    ...
    if (stateParam) {
      try {
        ...
        } else if (originalRedirectUri.includes("/api/auth/callback/google")) {
          // Self-redirect for testing
          clientType = "web-test";
    ...
    if (tokens.id_token) {
      ...
      } else if (clientType === "web-test") {
        console.log("✅ Web Test: Returning JSON response with tokens");
        return NextResponse.json({
          message: "Authentication successful! You can now use these tokens.",
          tokens: tokens,
